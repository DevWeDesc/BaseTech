...
export class GetUsersService {
      ...
      posts: [
        ...
        },
        {
          ...
          coments: [
            ...
            {
              accont: 'Facebook',
              content: 'Bonitão meu Developeeerrr!',
              id: 1,
              idUser: 1,
            },
            {
              accont: 'Facebook',
              content: 'Muito bem apresentavel Mark, parabéns',
              id: 1,
              idUser: 1,
            },
          ],
  ...
  getPostForId(
    idUser: number,
    idPost: number
  ): Observable<PostType | undefined> {
    let userFiltered = this.usersMock.find(
      (user) =>
        user.id == idUser && user.posts?.find((post) => post.id == idPost)
    );

    if (userFiltered) {
      console.log(`Usuario e post encontrado com Sucesso!`);
      return of(userFiltered.posts.find((post) => post.id == idPost));
    } else {
      console.log('Usuario ou post não encontrado');
      return EMPTY;
    }
  }
}
